...
internal class GoodsRemote {
    ...
    suspend fun getComments(
        token: String?,
        req: CommentListReq
    ): DataResult<CommonListResp<Comment>> {
    ...
    suspend fun getSkuAttributes(
        ...
        val dataResult = DataResult<MutableList<SkuAttribute>?>()
        try {
            val jsonObject = JSONObject()
            jsonObject.put("keyword", keyWord)
            val json = RxHttp.postJson(URLUtils.SKU_ATTRIBUTE + token)
                .addAll(jsonObject.toString())
                .toAwaitString()
                .await()
   ...
   suspend fun getSkuAttributeValues(
       ...
       val dataResult = DataResult<MutableList<SkuAttributeValue>?>()
       try {
           val jsonObject = JSONObject()
           jsonObject.put("keyword", keyWord)
           val json = RxHttp.postJson(URLUtils.SKU_ATTRIBUTE_VALUE + token)
               .addAll(jsonObject.toString())
               .toAwaitString()
               .await()
           ...
           if (json.contains(DataResult.TOKEN_PAST_LABEL)) {
               ...
               val resp = parseObject(
                   json,
                   SkuAttributeValueResp::class.java
               )
               dataResult.message = resp.message.info
               if (resp.message.code == DataResult.SERVICE_SUCCESS) {
                   dataResult.t = resp.data
    ...
    suspend fun getBrands(token: String?, keyword: String?): DataResult<MutableList<Brand>?> {

        val dataResult = DataResult<MutableList<Brand>?>()
        try {
            val jsonObject = JSONObject()
            jsonObject.put("keyword", keyword)
            val json = RxHttp.postJson(URLUtils.BRANDS + token)
                .addAll(jsonObject.toString())
                .toAwaitString()
                .await()
