...
public class IndexBarDataHelperImpl implements IIndexBarDataHelper {
     ...
     */
    @Override
    public IIndexBarDataHelper convert(List<CitySuspension> datas) {
//        if (null == datas || datas.isEmpty()) {
//            return this;
//        }
//        int size = datas.size();
//        for (int i = 0; i < size; i++) {
//            CitySuspension indexPinyinBean = datas.get(i);
...
//        }
     ...
     */
    @Override
    public IIndexBarDataHelper fillInexTag(List<CitySuspension> datas) {
        if (null == datas || datas.isEmpty()) {
            return this;
        }
        int size = datas.size();
    ...
    @Override
    public IIndexBarDataHelper sortSourceDatas(List<CitySuspension> datas) {
        if (null == datas || datas.isEmpty()) {
            return this;
        }
        convert(datas);
        fillInexTag(datas);
        //对数据源进行排序
        Collections.sort(datas, new Comparator<CitySuspension>() {
            @Override
            public int compare(CitySuspension lhs, CitySuspension rhs) {
    ...
    @Override
    public IIndexBarDataHelper getSortedIndexDatas(List<CitySuspension> sourceDatas, List<String> indexDatas) {
