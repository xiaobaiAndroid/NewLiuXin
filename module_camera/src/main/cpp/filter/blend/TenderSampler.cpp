...
#include "TenderSampler.h"

void TenderSampler::init() {
    BaseFilterSampler::init();
    mCurveTexture = new TextureProxy(GL_TEXTURE1);
    mGreyFrameTexture = new TextureProxy(GL_TEXTURE2);
}

string TenderSampler::getFragSrc() {
    return "#version 310 es\n"
           ...
           "\n"
           "    highp float xCoordinate = vTexCoords.x;\n"
           "    highp float yCoordinate = vTexCoords.y;\n"
           "\n"
           ...
           "\n"
           "    FragColor = vec4(base.r, base.g, base.b, 1.0);\n"
           "}";
}

void TenderSampler::setFilterImage(vector<NativeImage *> nativeImages) {
    ...
    if(!nativeImages.empty()){
        if(mGreyFrameTexture){
            mGreyFrameTexture->setImage2D(nativeImages[0]);
        }
    }
}

void TenderSampler::drawBefore() {
    if(mCurveTexture){
        string curveName = "curve";
        mCurveTexture->mapToFragShader(mProgramProxy->mProgram, curveName);
    }
    if(mGreyFrameTexture){
        string greyFrameName = "grey1Frame";
        mGreyFrameTexture->mapToFragShader(mProgramProxy->mProgram, greyFrameName);
    }
}

void TenderSampler::releaseGL() {
    ...
    if(mGreyFrameTexture){
        mGreyFrameTexture->destroy();
        delete mGreyFrameTexture;
        mGreyFrameTexture = nullptr;
    }
}