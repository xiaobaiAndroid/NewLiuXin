...
#include "SkinWhitenSampler.h"


void SkinWhitenSampler::init() {
    BaseFilterSampler::init();
    mCurveTexture = new TextureProxy(GL_TEXTURE1);
}

string SkinWhitenSampler::getFragSrc() {
    return "#version 310 es\n"
           ...
           "    // naver skin\n"
           "    textureColor = texture(uSourceTex, vTexCoords);\n"
           "    blurColor = gaussianBlur(uSourceTex);\n"
           "\n"
           "    //saturation\n"
           ...
           "\n"
           "    lowp float redCurveValue = texture(curve, vec2(textureColor.r, 0.0)).r;\n"
           "    lowp float greenCurveValue = texture(curve, vec2(textureColor.g, 0.0)).r;\n"
           "    lowp float blueCurveValue = texture(curve, vec2(textureColor.b, 0.0)).r;\n"
           "\n"
...
void SkinWhitenSampler::setFilterImage(vector<NativeImage *> nativeImages) {
    ...
    for (int i = 0; i < 256; i++){
        ...
        arrayOfByte[(2 + i * 4)] = ((unsigned char)arrayOfInt2[i]);
        arrayOfByte[(3 + i * 4)] = -1;
    }
...
void SkinWhitenSampler::drawBefore() {
    if(mCurveTexture){
...
void SkinWhitenSampler::releaseGL() {
