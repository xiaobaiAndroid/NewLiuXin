...
#include "WhiteCatSampler.h"

void WhiteCatSampler::init() {
    BaseFilterSampler::init();
    mCurveTexture = new TextureProxy(GL_TEXTURE1);
}

string WhiteCatSampler::getFragSrc() {
...
void WhiteCatSampler::drawBefore() {
    if(mCurveTexture){
...
void WhiteCatSampler::setFilterImage(vector<NativeImage *> nativeImages) {
    ...
    for (int j = 0; j < 256; j++){
        arrayOfByte[(1024 + j * 4)] = ((unsigned char)arrayOfInt4[j]);
...
void WhiteCatSampler::releaseGL() {
