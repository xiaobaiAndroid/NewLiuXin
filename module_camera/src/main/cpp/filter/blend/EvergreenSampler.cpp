...
#include "EvergreenSampler.h"

void EvergreenSampler::init() {
    BaseFilterSampler::init();
    mCurveTexture = new TextureProxy(GL_TEXTURE1);
}

string EvergreenSampler::getFragSrc() {
    return "#version 310 es\n"
           ...
           "    tColor = clamp(tColor, 0.0, 1.0);\n"
           "\n"
           "    redCurveValue = texture(curve, vec2(tColor.r, 0.0)).b;\n"
           "    greenCurveValue = texture(curve, vec2(tColor.g, 0.0)).b;\n"
           "    blueCurveValue = texture(curve, vec2(tColor.b, 0.0)).b;\n"
           "    redCurveValue = texture(curve, vec2(redCurveValue, 0.0)).r;\n"
           "    blueCurveValue = texture(curve, vec2(blueCurveValue, 0.0)).g;\n"
           "\n"
...
void EvergreenSampler::drawBefore() {
    if(mCurveTexture){
...
void EvergreenSampler::setFilterImage(vector<NativeImage *> nativeImages) {
...
void EvergreenSampler::releaseGL() {
