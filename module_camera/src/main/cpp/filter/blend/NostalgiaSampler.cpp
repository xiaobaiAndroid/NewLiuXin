...
#include "NostalgiaSampler.h"

void NostalgiaSampler::init() {
    BaseFilterSampler::init();
    mCurveTexture = new TextureProxy(GL_TEXTURE1);
    mCurve2Texture = new TextureProxy(GL_TEXTURE2);
}

string NostalgiaSampler::getFragSrc() {
...
void NostalgiaSampler::setFilterImage(vector<NativeImage *> nativeImages) {
    unsigned char arrayOfByte1[2048];
    ...
    for (int i = 0; i < 256; i++){
        arrayOfByte1[(0 + i * 4)] = ((unsigned char)arrayOfInt1[i]);
        arrayOfByte1[(1 + i * 4)] = ((unsigned char)arrayOfInt2[i]);
        arrayOfByte1[(2 + i * 4)] = ((unsigned char)arrayOfInt3[i]);
        arrayOfByte1[(3 + i * 4)] = ((unsigned char)arrayOfInt4[i]);
    }
    ...
    for (int j = 0; j < 256; j++){
        arrayOfByte1[(0 + (1024 + j * 4))] = ((unsigned char)arrayOfInt5[j]);
        arrayOfByte1[(1 + (1024 + j * 4))] = ((unsigned char)arrayOfInt6[j]);
        arrayOfByte1[(2 + (1024 + j * 4))] = ((unsigned char)arrayOfInt7[j]);
        arrayOfByte1[(3 + (1024 + j * 4))] = ((unsigned char)arrayOfInt4[j]);
    }
    ...
    curveImage.data = arrayOfByte1;
    if(mCurveTexture){
        mCurveTexture->setImage2D(&curveImage);
    }

    unsigned char arrayOfByte2[2048];
    ...
    for (int k = 0; k < 256; k++){
        arrayOfByte2[(0 + k * 4)] = ((unsigned char)arrayOfInt9[k]);
        arrayOfByte2[(1 + k * 4)] = ((unsigned char)arrayOfInt10[k]);
        arrayOfByte2[(2 + k * 4)] = ((unsigned char)arrayOfInt11[k]);
        arrayOfByte2[(3 + k * 4)] = ((unsigned char)arrayOfInt8[k]);
    }
    NativeImage curve2Image;
    curve2Image.width = 256;
    curve2Image.height = 1;
    curve2Image.format = GL_RGBA;
    curve2Image.data = arrayOfByte2;

    if(mCurve2Texture){
        mCurve2Texture->setImage2D(&curve2Image);
    }

}

void NostalgiaSampler::drawBefore() {
    if(mCurveTexture){
        string curveName = "curve";
        mCurveTexture->mapToFragShader(mProgramProxy->mProgram, curveName);
    }
    if(mCurve2Texture){
        string curve2Name = "curve2";
        mCurve2Texture->mapToFragShader(mProgramProxy->mProgram, curve2Name);
    }
    ...
    ShaderProxy::setFloat(mProgramProxy->mProgram,"blurSize",1.0f);
}

void NostalgiaSampler::releaseGL() {
    ...
    if(mCurve2Texture){
        mCurve2Texture->destroy();
        delete mCurve2Texture;
        mCurve2Texture = nullptr;
    }
}