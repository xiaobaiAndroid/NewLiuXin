...
class CommentDetailViewModel: BaseViewModel() {
    ...
    fun comment(content: String, commentParentId: String?, dynamic: Dynamic?) = viewModelScope.launch(Dispatchers.IO) {
        val commentReq = CommentReq()
        commentReq.content = content
        commentReq.replyUserId = dynamic!!.userId
        commentReq.mediaId = dynamic.id
        commentReq.commentId = commentParentId
    ...
    val commentLD: MutableLiveData<Comment?> by lazy {
    ...
    fun getChildCommentData() = viewModelScope.launch(Dispatchers.IO) {
        req.queryObj?.commentId = commentLD.value?.id ?: ""
