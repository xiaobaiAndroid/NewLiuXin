...
void main()
{
    ...
    vec4 textureColor = texture( uSourceTex, vec2(xCoordinate, yCoordinate));
    ...
    lowp vec4 fragmentColor = texture(uSourceTex, vec2(centerTextureCoordinate.x, centerTextureCoordinate.y)) * 0.2270270270;
    fragmentColor += texture(uSourceTex, vec2(oneStepLeftTextureCoordinate.x, oneStepLeftTextureCoordinate.y)) * 0.3162162162;
    fragmentColor += texture(uSourceTex, vec2(oneStepRightTextureCoordinate.x, oneStepRightTextureCoordinate.y)) * 0.3162162162;
    fragmentColor += texture(uSourceTex, vec2(twoStepsLeftTextureCoordinate.x, twoStepsLeftTextureCoordinate.y)) * 0.0702702703;
    fragmentColor += texture(uSourceTex, vec2(twoStepsRightTextureCoordinate.x, twoStepsRightTextureCoordinate.y)) * 0.0702702703;
    ...
    // step3 curve
    highp float redCurveValue = texture(curve, vec2(textureColor.r, 0.0)).r;
    highp float greenCurveValue = texture(curve, vec2(textureColor.g, 0.0)).g;
    highp float blueCurveValue = texture(curve, vec2(textureColor.b, 0.0)).b;
    ...
    // step6 curve
    redCurveValue = texture(curve2, vec2(redCurveValue, 1.0)).r;
    greenCurveValue = texture(curve2, vec2(greenCurveValue, 1.0)).g;
    blueCurveValue = texture(curve2, vec2(blueCurveValue, 1.0)).b;

    // step7 curve
    redCurveValue = texture(curve, vec2(redCurveValue, 1.0)).b;
    greenCurveValue = texture(curve, vec2(greenCurveValue, 1.0)).b;
    ...
    FragColor =  vec4(BCSColor.r,BCSColor.g,BCSColor.b,1.0);
}
