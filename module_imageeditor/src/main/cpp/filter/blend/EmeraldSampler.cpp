...
#include "EmeraldSampler.h"

void EmeraldSampler::init(JNIEnv *env, jobject *assetsManager) {
    string vertPath = "glsl/common_vert.glsl";
    string fragPath = "glsl/emerald_frag.glsl";
    BaseFilterSampler::initCommon(env,assetsManager,vertPath,fragPath);

    mCurveTexture = new TextureProxy(GL_TEXTURE1);
}

void EmeraldSampler::drawBefore() {
    if(mCurveTexture){
...
void EmeraldSampler::setFilterImage(vector<NativeImage *> nativeImages) {
    ...
    for (int j = 0; j < 256; j++){
        ...
        arrayOfByte[(1 + (1024 + j * 4))] = ((unsigned char)arrayOfInt4[j]);
