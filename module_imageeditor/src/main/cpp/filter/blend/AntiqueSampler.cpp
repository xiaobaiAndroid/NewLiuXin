...
#include "AntiqueSampler.h"


void AntiqueSampler::init(JNIEnv *env, jobject *assetsManager) {
    string vertPath = "glsl/common_vert.glsl";
    string fragPath = "glsl/antique_frag.glsl";
    BaseFilterSampler::initCommon(env,assetsManager,vertPath,fragPath);

    mCurveTexture = new TextureProxy(GL_TEXTURE1);
}

void AntiqueSampler::drawBefore() {
    if(mCurveTexture){
...
void AntiqueSampler::setFilterImage(vector<NativeImage *> nativeImages) {
