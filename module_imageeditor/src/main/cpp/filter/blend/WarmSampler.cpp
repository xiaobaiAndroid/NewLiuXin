...
#include "WarmSampler.h"


void WarmSampler::init(JNIEnv *env, jobject *assetsManager) {
    ...
    mLayerImageTexture = new TextureProxy(GL_TEXTURE3);
}

void WarmSampler::drawBefore() {
    if(mCurveTexture){
        string curveName = "curve";
        mCurveTexture->mapToFragShader(mProgramProxy->mProgram, curveName);
    }
    if(mGreyFrameTexture){
        string greyFrameName = "greyFrame";
        mGreyFrameTexture->mapToFragShader(mProgramProxy->mProgram, greyFrameName);
    }
    if(mLayerImageTexture){
        string layerImageName = "layerImage";
        mLayerImageTexture->mapToFragShader(mProgramProxy->mProgram,layerImageName);
    }
}

void WarmSampler::setFilterImage(vector<NativeImage *> nativeImages) {
    ...
    if(nativeImages.size() == 2){
        ...
        if(mLayerImageTexture){
            mLayerImageTexture->setImage2D(nativeImages[1]);
        }
    }
}

void WarmSampler::releaseGL() {
    ...
    if(mLayerImageTexture){
        mLayerImageTexture->destroy();
        delete mLayerImageTexture;
        mLayerImageTexture = nullptr;
    }
}