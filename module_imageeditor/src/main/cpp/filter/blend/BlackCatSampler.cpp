...
#include "BlackCatSampler.h"

void BlackCatSampler::init(JNIEnv *env, jobject *assetsManager) {
    string vertPath = "glsl/common_vert.glsl";
    string fragPath = "glsl/black_cat_frag.glsl";
    BaseFilterSampler::initCommon(env,assetsManager,vertPath,fragPath);

    mCurveTexture = new TextureProxy(GL_TEXTURE1);
}

void BlackCatSampler::drawBefore() {
    if(mCurveTexture){
...
void BlackCatSampler::setFilterImage(vector<NativeImage *> nativeImages) {
    ...
    for (int j = 0; j < 256; j++){
        ...
        arrayOfByte[(2 + (1024 + j * 4))] = ((unsigned char)arrayOfInt6[j]);
...
void BlackCatSampler::releaseGL() {
