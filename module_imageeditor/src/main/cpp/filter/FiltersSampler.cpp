...
void FiltersSampler::init(JNIEnv *env, jobject *assetsManager) {
    for (auto &mFilterSampler: mFilterSamplers) {
        mFilterSampler->init(env, assetsManager);
    }
    initFlipVertex();
...
void FiltersSampler::initFrameBuffers() {
    ...
    mFrameBuffers = vector<GLuint>(mFilterSamplers.size(), 0);

    for (int i = 0; i < mFrameBuffers.size(); ++i) {
        //初始化帧缓冲
        glGenFramebuffers(1, &(mFrameBuffers.at(i)));
        glBindFramebuffer(GL_FRAMEBUFFER, mFrameBuffers.at(i));
...
void FiltersSampler::deleteFrameBuffers() {
    for (auto framebufferId: mFrameBuffers) {
        if (framebufferId) {
...
void FiltersSampler::draw(VAOProxy &vao) {
    if(mFrameBuffers.empty() || !mSourceTex){
        return;
    }
    ...
    //离屏渲染
    //先渲染滤镜纹理，再把渲染好的滤镜纹理（再把帧缓冲区关联的纹理附件）依次传给亮度采样器渲染。依次循环叠加效果
    ...
    for (int i = 0; i < mFilterSamplers.size(); ++i) {
        ...
        if (i == 0) {
            mFilterSamplers.at(i)->draw(previewTexture, mFlipVAO, mModelMatrix);
        } else {
            mModelMatrix.scale = glm::vec3(1.0f);
            mFilterSamplers.at(i)->draw(previewTexture, &vao, mModelMatrix);
        }
        previewTexture = mTextures.at(i);
    }
}

void FiltersSampler::setFilter(JNIEnv *env, jobject *asset, int filter_type) {
