...
public class MyPlayerView extends FrameLayout implements AdViewProvider {
  ...
  public MyPlayerView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    //播放布局
    int playerLayoutId = R.layout.my_player_view;
    //是否使用插图
      ...
      TypedArray a =
          context
              ...
              .obtainStyledAttributes(
                  attrs, R.styleable.MyPlayerView, defStyleAttr, /* defStyleRes= */ 0);
      try {
        shutterColorSet = a.hasValue(R.styleable.MyPlayerView_shutter_background_color);
        shutterColor =
            a.getColor(R.styleable.MyPlayerView_shutter_background_color, shutterColor);
        playerLayoutId =
            a.getResourceId(R.styleable.MyPlayerView_player_layout_id, playerLayoutId);
        useArtwork = a.getBoolean(R.styleable.MyPlayerView_use_artwork, useArtwork);
        defaultArtworkId =
            a.getResourceId(R.styleable.MyPlayerView_default_artwork, defaultArtworkId);
        useController = a.getBoolean(R.styleable.MyPlayerView_use_controller, useController);
        surfaceType = a.getInt(R.styleable.MyPlayerView_surface_type, surfaceType);
        resizeMode = a.getInt(R.styleable.MyPlayerView_resize_mode, resizeMode);
        controllerShowTimeoutMs =
            a.getInt(R.styleable.MyPlayerView_show_timeout, controllerShowTimeoutMs);
        controllerHideOnTouch =
            a.getBoolean(R.styleable.MyPlayerView_hide_on_touch, controllerHideOnTouch);
        controllerAutoShow =
            a.getBoolean(R.styleable.MyPlayerView_auto_show, controllerAutoShow);
        showBuffering = a.getInteger(R.styleable.MyPlayerView_show_buffering, showBuffering);
        keepContentOnPlayerReset =
            a.getBoolean(
                R.styleable.MyPlayerView_keep_content_on_player_reset,
                keepContentOnPlayerReset);
        controllerHideDuringAds =
            a.getBoolean(R.styleable.MyPlayerView_hide_during_ads, controllerHideDuringAds);
      } finally {
        a.recycle();
      }
    }
    ...
    if (contentFrame != null && surfaceType != SURFACE_TYPE_NONE) {
      ...
      // We don't want surfaceView to be clickable separately to the MyPlayerView itself, but we
      // do want to register as an OnClickListener so that surfaceView implementations can propagate
      // click events up to the MyPlayerView by calling their own performClick method.
    ...
    if (customController != null) {
      ...
      // Propagate attrs as playbackAttrs so that MyPlayerControlView's custom attributes are
  ...
  public static void switchTargetView(
      Player player,
      @Nullable MyPlayerView oldPlayerView,
      @Nullable MyPlayerView newPlayerView) {
   ...
   * {@link #switchTargetView(Player, MyPlayerView, MyPlayerView)} rather than this method.
  ...
  /**
   * Sets the {@link MyPlayerControlView.VisibilityListener}.
   *
   * <p>If {@code listener} is non-null then any listener set by {@link
   * #setControllerVisibilityListener(MyPlayerControlView.VisibilityListener)} is removed.
   *
   ...
   * #setControllerOnFullScreenModeChangedListener(MyPlayerControlView.OnFullScreenModeChangedListener)}.
   *
   ...
   */
  @SuppressWarnings(
      "deprecation") // Calling the deprecated method on MyPlayerControlView for now.
  ...
  /**
   * Sets the {@link MyPlayerControlView.OnFullScreenModeChangedListener}.
   *
   * <p>Clears any listener set by {@link
  ...
  public void setControllerOnFullScreenModeChangedListener(
      @Nullable MyPlayerControlView.OnFullScreenModeChangedListener listener) {
    Assertions.checkStateNotNull(controller);
  ...
  */
  ...
  // Implementing the deprecated MyPlayerControlView.VisibilityListener and
  // MyPlayerControlView.OnFullScreenModeChangedListener for now.
  @SuppressWarnings("deprecation")
  private final class ComponentListener
      implements Player.Listener,
          ...
          MyPlayerControlView.OnFullScreenModeChangedListener {
    ...
    public void onTracksChanged(Tracks tracks) {
      ...
      Player player = checkNotNull(MyPlayerView.this.player);
      Timeline timeline =
          player.isCommandAvailable(COMMAND_GET_TIMELINE)
    ...
    // MyPlayerControlView.VisibilityListener implementation
    ...
    // MyPlayerControlView.OnFullScreenModeChangedListener implementation
